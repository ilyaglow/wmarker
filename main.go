...
func WaterMark(img image.Image, markText string) (image.Image, error) {
	...
	diagonal := vg.Length(math.Sqrt(float64(w*w*3 + h*h*3)))
	...
	fontStyle, _ := vg.MakeFont("Helvetica", vg.Inch*0.7)
	...
	c.SetColor(color.RGBA{0, 0, 0, 25})
...
func writeTo(img image.Image, ext string) (rv *bytes.Buffer, err error) {
	...
	switch ext {
	case ".jpg", ".jpeg":
		err = jpeg.Encode(rv, img, &jpeg.Options{Quality: 90})
	case ".png":
